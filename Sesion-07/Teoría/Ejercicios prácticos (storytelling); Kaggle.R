...
puntuacionbinaria <- dcast(binaryratings, movieId~userId, value.var = "rating", na.rm=FALSE)
...
pelisconpuntuacion <- length(unique(ratings$movieId)) # 9,724
pelis2 <- pelis[-which((pelis$movieId %in% ratings$movieId) == FALSE),]
rownames(pelis2) <- NULL

# Borramos las pelis sin puntuacion de nuestra matriz generos2
matriz_generos3 <- matriz_generos2[-which((pelis$movieId %in% ratings$movieId) == FALSE),]
...
ggplot(tabla_vis[1:6, ], aes(x = movie, y = views)) +
  ...
  scale_x_discrete(labels=subset(pelis2, pelis2$movieId == table_views$movie)$title) +
